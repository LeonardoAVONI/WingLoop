...
class WingLoop:
    def __init__(self):
        ...
        Initializes a class responsibe of handling an ASWING instance
        """
        pass
    ...
    def Launch_ASWING(self,aswing_path,sim_directory,asw_filename,print_output,timer_text=0.000001,finished_writing_check_timestep=0.001):
        ...
        asw_filename: .asw filename to use
        print_output: True or False: if True, WingLoop data will be printed to the console
        check_timestep: timer used to check for the last update on the size of the state file written by ASWING
        timer_text: timer used by default for the Aswing_Director functions
        """
    ...
    def Load_Files(self,filename):
        """_summary_
        From the main ASWING menu, this function loads the file called "filename", according to its extension
        When using ASWING from the terminal, any file extension can be loaded. Here the extension will make the difference
...
# Deactivate Graphics
    ...
    def Time_Transient_Simulation(self,Dt,N,K):
        """
        After having loaded the needed settings, and having imported the .state file 
        for the trimming point infos,
        the program moves in OPER
        changes the simulation to time-transient
        and solves it by looping it with Python
        """
